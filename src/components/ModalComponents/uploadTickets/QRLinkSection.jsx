...
const QRLinksSection = React.forwardRef(({ 
  ...
  const handleCopyToClipboard = useCallback(async (text, ticketIndex, linkType) => {
    try {
  ...
  React.useImperativeHandle(ref, () => ({
    ...
    updateTicket: (index, linkType, value) => {
      if (index >= 0 && index < maxQuantity && !ticketLinks[index]?.isExisting) {
  ...
  }), [getCurrentData, hasData, isValid, getCompletedCount, isTicketComplete, maxQuantity, initialData, handleLinkChange, ticketLinks, extractQRLinksFromUploadTickets]);
  ...
  const renderLinkField = (ticketIndex, linkType, label, placeholder) => {
    const link = ticketLinks[ticketIndex] || { qr_link_android: "", qr_link_ios: "", isExisting: false };
      ...
      return (
        <div>
          ...
          <div className="relative">
            ...
            <button
              type="button"
              onClick={() => handleCopyToClipboard(value, ticketIndex, linkType)}
    ...
    return (
      <div>
        ...
        <input
          ...
          value={value}
          onChange={(e) => handleLinkChange(ticketIndex, linkType, e.target.value)}
        ...
        {maxQuantity === 0 ? (
          ...
          <div className="space-y-4">
            {Array.from({ length: maxQuantity }, (_, index) => {
              ...
              const hasExistingData = link?.isExisting && (link?.qr_link_android || link?.qr_link_ios);
